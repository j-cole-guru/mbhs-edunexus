...
const ArchiveStudents = () => {
  ...
  const fetchStudents = async () => {
    ...
    setSuccess('')
    try {
      const data = await apiFetch(`/students?select=*&class_id=eq.${filterClass}&is_active=eq.true&order=full_name`)
      setStudents(data || [])
  ...
  const handleArchive = async () => {
    ...
    try {
      // Archive students in a loop since standard PostgREST PATCH doesn't easily support multiple IDs with different data
      // but here they all share the same graduation year and reason.
      // We can use 'id=in.(id1,id2,...)'
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Bulk archive students by class for graduation or withdrawal.</p>
        </div>
      </div>
      ...
      {students.length === 0 && filterClass && !fetching && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <Users className="h-12 w-12 mx-auto text-gray-300 mb-4" />
          <h3 className="text-lg font-medium text-gray-900 mb-1">No active students found</h3>
          <p className="text-gray-500">All students in this class might already be archived.</p>
        </div>
      )}
