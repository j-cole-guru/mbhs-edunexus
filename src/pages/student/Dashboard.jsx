  ...
  const loadData = async (s) => {
    try {
      ...
      const [classRes, levelRes, termRes, attendanceRes] = await Promise.all([
        s.class_id ? fetch(`${BASE_URL}/classes?id=eq.${s.class_id}&select=name`, { headers }) : Promise.resolve(null),
        s.level_id ? fetch(`${BASE_URL}/levels?id=eq.${s.level_id}&select=name`, { headers }) : Promise.resolve(null),
        fetch(`${BASE_URL}/terms?is_current=eq.true&select=name,year`, { headers }),
        fetch(`${BASE_URL}/attendance?student_id=eq.${s.id}&select=status`, { headers })
      ])
      ...
      setCurrentTerm(terms[0] ? `${terms[0].name} ${terms[0].year}` : 'No active term')
    ...
    <div className="p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Student Profile: {student.full_name}</p>
        </div>
        <div className="mt-4 md:mt-0 bg-blue-50 px-4 py-2 rounded-lg border border-blue-100 flex items-center gap-3">
          <Clock className="h-5 w-5 text-blue-600" />
          <span className="text-sm font-medium text-blue-800">{currentTerm}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-gray-900">{student.student_number}</p>
          </div>
        </div>
        ...
        <div className="bg-blue-900 rounded-xl shadow-lg p-6 text-white">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="bg-blue-800 px-2 py-1 rounded text-xs">{stats.rate}%</span>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div className="pt-6 border-t border-gray-100">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm text-gray-500">{stats.present + stats.absent + stats.late}</span>
                </div>
        ...
        <div className="bg-gradient-to-br from-blue-800 to-indigo-900 rounded-xl shadow-lg p-8 text-white flex flex-col justify-between">
          <div>
            ...
            <p className="text-blue-100 leading-relaxed mb-6">
              Maintaining high attendance is key to your academic progress at MBHS. 
